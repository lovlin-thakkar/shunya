...
def _format_transcript(turns):
    lines = []
    for t in turns:
        quirk_note = f" [{chr(44).join(t['quirks'])}]" if t.get("quirks") else ""
...
def evaluate_result(test_result_id: str, rubric: dict):
    try:
        result = TestResult.objects.select_related("test_run__scenario", "test_run__agent").get(id=test_result_id)
    ...
    message = client.messages.create(
        ...
        system=JUDGE_SYSTEM_PROMPT, messages=[{"role": "user", "content": user_prompt}],
    )
