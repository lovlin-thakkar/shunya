...
async def _notify_django(path: str, payload: dict):
    try:
        async with httpx.AsyncClient(timeout=5) as c:
            await c.post(f"{DJANGO_API_URL}/internal/{path}", json=payload)
...
async def run_voice_agent(
    ...
    async with aiohttp.ClientSession() as aiohttp_session:
        await _run_voice_agent_inner(
            ...
            greeting, schema_name, aiohttp_session, ready_event,
        )
...
async def _run_voice_agent_inner(
    ...
    async def on_participant_joined(transport, participant):
        ...
        async with httpx.AsyncClient(timeout=5) as c:
            r = await c.post(f"{DJANGO_API_URL}/internal/calls/start/", json=data)
            if r.status_code == 201:
                call_resp["call_id"] = r.json()["call_id"]

        # If the agent has a greeting, trigger it to speak when the first participant
        # joins. We push a user turn that causes the LLM to produce audio via TTS.
        # The system prompt's greeting instruction ensures the LLM says the right thing.
        if greeting:
    ...
    async def on_participant_left(transport, participant, reason):
        if call_resp["call_id"]:
            await _notify_django(
                ...
                {"reason": reason, "schema_name": schema_name},
            )
        await task.cancel()
